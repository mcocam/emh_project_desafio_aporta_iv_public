...
export const KpiMortalityCard = ({title, apiPath}) => {
    ...
    if (!data){
        ...
        <Grid item xs={12} sm={6} md={4}>
            <Paper sx={{ padding: "1rem", opacity: isFetching ? 0.5: 1, transition: 'opacity 0.4s'}} elevation={3}>
                ...
                <Typography sx={{textAlign: 'center', color: '#252525', marginTop: '1rem', fontSize: '0.8rem'}}>Exitus observados/esperados</Typography>
                <Typography sx={{fontWeight: 'bold', fontSize: '1.5rem', textAlign: 'center'}}>{prettifyNumber(roundTo(data.entitiesData.filter(d => d.target_entities === selection.selectedEntity).map(d => d.observed_exitus)[0] || 0,0)) + ' / ' + prettifyNumber(roundTo(data.entitiesData.filter(d => d.target_entities === selection.selectedEntity).map(d => d.expected_exitus)[0] || 0,0))}</Typography>
                <Box>
