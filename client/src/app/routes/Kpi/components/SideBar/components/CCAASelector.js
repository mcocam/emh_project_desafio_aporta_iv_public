...
export const CCAASelector = () => {

    const dispatch = useDispatch();
    const entities = useSelector( (state) => state.catalogue.catalogue.ccaa_hosp );
    const selectedEntity = useSelector( (state) => state.kpi.selectedEntity );

    const handleChange = (event) => {
        dispatch(kpiSetEntity(event.target.value));
    }
    ...
    if (!selectedEntity || !entities){
        ...
        return(
            <Box sx={boxStyles}>
                <FormControl fullWidth>
                    <InputLabel id="desc-entity-label">Comunidad</InputLabel>
                    <Select
