...
import { kpiSetccsr } from "../../../../../redux/slices/kpiSlice/kpiSlice";
...
export const CCSRSelector = () => {
    ...
    const selectedCCSR = useSelector( (state) => state.kpi.ccsr );
    const selectedChapters = useSelector( (state) => state.kpi.chapters );

    const handleChange = (event) => {
        dispatch(kpiSetccsr(event));
    }
    ...
    useEffect( () => {
        if (dxList && selectedChapters){
            ...
            dispatch(kpiSetccsr(uniqueCCSR));
        }
