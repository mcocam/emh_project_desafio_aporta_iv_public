...
import { kpiSetAges } from '../../../../../redux/slices/kpiSlice/kpiSlice';
...
export const AgeSelector = () => {
    ...
    const selectedAges = useSelector( (state) => state.kpi.age );

    useEffect(()=> {
        dispatch(kpiSetAges(ageGroups));
    }, 
    [ageGroups, dispatch]);
    ...
    const handleChange = (event) => {
        ...
        dispatch(kpiSetAges(selectedAges.length === ageGroups.length ? [] : ageGroups));
            return;
        }
        dispatch(kpiSetAges(event.target.value));
    }
