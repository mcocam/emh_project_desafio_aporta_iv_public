...
export const Top10ExitusTable = ({data}) => {
    return(
        <Box sx={{height: '20rem'}}>
            <TableContainer sx={{height: '100%', width: '100%'}}>
                <Table aria-label="Top 10 CCSR Average Stay" size="small">
                    ...
                    {data.map((d) => (
                        ...
                        >
                            <TableCell scope="row" size="small" sx={{fontSize: '0.7rem', maxWidth: '10rem'}}>{d.ccsr}</TableCell>
                            <TableCell align="center" size="small" sx={{fontSize: '0.7rem'}}>{prettifyNumber(roundTo(d.discharges, 0))}</TableCell>
                            <TableCell align="center" size="small" sx={{fontSize: '0.7rem'}}>{prettifyNumber(roundTo(d.exitus*100, 2))+'%'}</TableCell>
                        </TableRow>
                    ))}
                    </TableBody>
                </Table>
