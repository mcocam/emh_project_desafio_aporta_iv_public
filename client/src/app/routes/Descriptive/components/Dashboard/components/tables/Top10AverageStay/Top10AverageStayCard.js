...
import { Top10AverageStayTable } from './Top10AverageStayTable';


export const Top10AverageStayCard = () => {
    ...
    useEffect( () => {
        ...
        if (!delay){
            if (!checkEmptyArrays(selection) && !selection.levelChange){
                    fetchData('api/descriptive/top10avgstay', selection, setIsFetching, setData);
            }
        }
    ...
    if (!data){
        ...
        <Grid item xs={12} sm={6}>
            ...
            <Typography sx={{textAlign: 'center', marginBottom: '0.5rem'}}>Top 10 CCSR por estancia media (días)</Typography>
            <Top10AverageStayTable data={data}/>
            </Paper>
        </Grid>
        )
    }
