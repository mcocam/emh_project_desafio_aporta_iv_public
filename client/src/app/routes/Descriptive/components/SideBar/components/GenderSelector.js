...
import { descSetGender } from '../../../../../redux/slices/descriptiveSlices/descriptiveSlice';
...
export const GenderSelector = () => {
    ...
    const selectedGenders = useSelector( (state) => state.descriptive.gender );
    
    const handleChange = (event) => {
        if (event.target.value.length === 0){
            dispatch(descSetGender(genderList));
        }else{
            dispatch(descSetGender(event.target.value));
        }
    } 
